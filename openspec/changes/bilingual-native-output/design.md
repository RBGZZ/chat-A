...
1. 哨兵串的最终形态(可读性 vs 抗冲突)——apply 时定,进 golden。
2. desktop send/onToken 真实编排形状(截流落点)——apply 前先读 `main.ts` 核定。
3. 语音模式(voice-loop)双语原生何时接——独立后续。
4. 是否也让"显示语种正文"在双语态可选地不流式(等整段拆好再显示)以简化——倾向否(保流式),除非 onToken 截流过脆。
