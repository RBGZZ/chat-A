...
- **随心情**:`CHAT_A_TTS_EMOTION_FROM_MOOD=on` → 朗读按小雪当前 PAD 心情自动注入情绪指令(开心→上扬、低落→低落语气…,只用不伤音色的安全情绪词),复刻音色逐回合随情绪变。默认 off=回落静态、零回归。
  - ⚠️ 要让情绪**真随对话起伏**,需 `CHAT_A_APPRAISER=llm`(用 LLM 评估每轮情绪;默认关键词 appraiser 几乎不动)。desktop 现已接此开关(经 assembleApp;此前仅 cli 生效),且修复了 mood 读取 stale 快照的问题(回合后从持久化 store 重载活 PAD)。
...
- `CHAT_A_TTS_STREAM_READOUT=on` → desktop 朗读改**同会话流式喂**:回复逐句喂进同一条 CosyVoice 会话(同一 `run-task` 多次 `continue-task`),首句到齐即合成出声、不等整段,且复刻音色不漂移(2026-06-25 真机验证:同会话逐句 ≈ 整段、不飘)。默认 off=沿用整段一次合成、零回归。
- 生效条件:门控开 **且** TTS 引擎支持同会话流式喂(目前仅 **cosyvoice**;其它引擎门控开了也回落整段)。流式合成抛错 → 自动降级整段一次合成,不崩(§3.2)。
- **两步**:① **同语种(无翻译)** 把朗读挂到回合 token 流(`onToken`)边生成边句切边喂——**首句即出声,真正解 R7 音频比文字慢几秒**;② 异语种(`CHAT_A_TTS_LANG` 与显示语种不同,需翻译)翻译仍需整段,翻译后再句切流式喂——**流式化合成段但起步仍受翻译延迟**(根治见下「双语原生输出」直接出合成语种)。
- 句切用 runtime `SentenceSplitter`(CJK 友好,句末标点切句、超长软切/硬切)。打断(新消息/点停)直接 close 流式会话丢弃在途音频(不发 finish-task)+ 清渲染层队列。
- 复用关系:本流式 API(`CosyVoiceTts.synthesizeStream`)供下「双语原生输出」与 VoiceLoop 同会话流式改造复用。

#### 双语原生输出 / 音频优先(异语种免翻译阻塞,默认 off)

- `CHAT_A_TTS_DUAL_OUTPUT=on` + 显示语种(`CHAT_A_DISPLAY_LANG`)≠ 合成语种(`CHAT_A_TTS_LANG`)→ 进 **audio-first** 路:**回复本身直接用合成语种**(如显示中文、`CHAT_A_TTS_LANG=ja` → 主 LLM 直接产日语回复),desktop 逐句**直接**流式喂 TTS——**首音最快、音频路彻底无翻译往返**(原生口语、不生硬)。中文显示由 desktop 在音频之后翻译**覆盖**气泡(文字次要、不挡音频)。
- 生效条件:开关开 **且** 朗读开 **且** TTS 可用 **且** 引擎支持同会话流式喂(cosyvoice)**且** 显示≠合成语种。任一不满足 → 逐字回落上面的流式朗读/翻译通道(零回归)。
- 与 `CHAT_A_TTS_STREAM_READOUT` 关系:audio-first 复用同一套同会话流式喂(`makeTokenStreamReadout`),但**回复即合成语种**故连翻译那趟都省了(`STREAM_READOUT` 的异语种② 仍要先翻译)。开 `DUAL_OUTPUT` 时不必再纠结②。
- 降级:音频流式失败 → 整段一次合成兜底;翻译失败 → 气泡保留合成语种原文(有字尽力)。`[timing] 首音=…ms` 日志可观测首音延迟。
- 注:另有一套「主 LLM 一次产『显示正文 ⟦哨兵⟧ 合成语种口语版』」的哨兵机制已实现 + 测试覆盖,但因当前模型(qwen-plus)不稳定遵从该格式而**默认休眠不接线**,留作能遵从格式的更强模型的扩展点(详见 `openspec/changes/bilingual-native-output/design.md` 实施转向)。

### 各路径能跑到什么程度(如实说明)

- **100% key-only(`pnpm test:voice`)**:文本→云 LLM→云 TTS→WAV。只需 key + 网络,**绝对可跑**。
- **全语音(WAV→云 STT→云 LLM→云 TTS→WAV)**:设 `CHAT_A_AUDIO_DEVICE=wav`、`CHAT_A_STT_KIND=qwen`、`CHAT_A_VAD=energy`、`CHAT_A_VOICE=1` 即可接通;其中**云 STT(`qwen3-asr-flash` 经 OpenAI 兼容端点的 input_audio Data URL 往返)已真网络验证通过**(2026-06-25 `pnpm smoke:asr`:WAV→转写文本+情绪+检测语种→PAD 拉力 闭环跑通)。
...
### 情绪调优旋钮(行为即配置,默认 = 现值零回归)

调"情绪有多容易动 / 冷启动有多保守"。不设则全部回落默认(逐字现状);非法/越界值逐字段回落默认。

- `CHAT_A_COLD_START_TURNS`(默认 5):冷启动窗口轮数,窗口内情绪幅度减半 + 加速回弹。设 `0` = **关闭冷启动压制**(首轮起情绪即正常起伏;desktop 内存后端每次重启 turn 归 0、易困在冷启动时尤其有用)。
- `CHAT_A_COLD_START_REBOUND`(默认 2):冷启动期回弹加速系数(放大弹簧 k)。
- `CHAT_A_EMOTION_PLEASURE_THRESHOLD`(默认 0.35,[0,1]):PAD→离散情绪的 Pleasure 阈值。**调低 = 情绪更易跨出 neutral**(更容易判为开心/低落)。
- `CHAT_A_EMOTION_AROUSAL_THRESHOLD`(默认 0.25,[0,1]):Arousal 阈值,分高/低唤起(joyful↔content、irritated↔down)。

> 注:这组阈值是**单一权威**——同时驱动显示情绪、喂 LLM 的「当前情绪」系统提示文案、以及复刻音色的语音情绪指令(三处一致,不漂移)。负面人际姿态(posture)的触发边界 `-0.35` **独立**、不随此阈值联动。
