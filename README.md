...
- **两步**:① **同语种(无翻译)** 把朗读挂到回合 token 流(`onToken`)边生成边句切边喂——**首句即出声,真正解 R7 音频比文字慢几秒**;② 异语种(`CHAT_A_TTS_LANG` 与显示语种不同,需翻译)翻译仍需整段,翻译后再句切流式喂——**流式化合成段但起步仍受翻译延迟**(根治待 parked 的 bilingual-native-output 直接出目标语种)。
- 句切用 runtime `SentenceSplitter`(CJK 友好,句末标点切句、超长软切/硬切)。打断(新消息/点停)直接 close 流式会话丢弃在途音频(不发 finish-task)+ 清渲染层队列。
- 复用关系:本流式 API(`CosyVoiceTts.synthesizeStream`)后续供 parked 的 bilingual-native-output 与 VoiceLoop 同会话流式改造复用。
