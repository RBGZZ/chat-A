...
// ⚠️ 本文件被打进 **preload.cjs**(沙箱,无 node:async_hooks)→ **绝不能 import @chat-a/runtime 的值**
// (会把 runtime→@opentelemetry/api→node:async_hooks 拖进 preload,导致 preload 加载失败、渲染层拿不到 API)。
// 故双语哨兵/显示段抽取在此**本地定义**(与 cognition 的 DUAL_OUTPUT_SENTINEL/extractDisplaySegment 同值);
// 防漂移由 ipc-contract 测试 import cognition 常量断言相等钉死。
/** 双语原生输出哨兵(本地副本,= cognition DUAL_OUTPUT_SENTINEL)。 */
export const DUAL_OUTPUT_SENTINEL = '⟦SPOKEN⟧';
/** 取双语回复显示段(哨兵**后**;音频优先排序=口语在前;= cognition extractDisplaySegment)。无哨兵→原文 trim(降级安全)。 */
...
export async function runSendTurn(port: SendTurnPort, text: string): Promise<void> {
  try {
    ...
    // 定型气泡:先取显示段(双语态切哨兵;无哨兵=原样)再去括号(逐 token 期间可能短暂闪现哨兵/旁白,定型替换为干净文本)。
    port.emit(IPC.reply, stripStageDirections(extractDisplaySegment(reply)));
  } catch (err) {
...
/**
 * 去除舞台指示/旁白(确定性兜底,§4.1)。`StyleDisciplineContributor` 提示词已要求 LLM 不写括号动作,
 * 但模型偶尔无视、且 desktop 文本朗读路不经 runtime classifier-processor(只语音模式用)——此纯函数保证
 * 括号内的动作/神态**绝不进 TTS、也不留在定型气泡**(用户实测:漏网括号既显示又被读出来)。
 *
 * 仅剥「旁白形态」:全/半角圆括号 （…）(…)、方/角/方头括号 【…】〔…〕［…］、星号动作 *…*。
 * **保留** 「」『』(日文对话/强调引号,非旁白)与正常标点;剥后收敛多余空格/空行并 trim。
 */
...
export async function runSpeakReply(
  ...
  // 先去括号旁白再翻译/合成:既不让旁白进 TTS,也省得翻译那段无用文字。
  const clean = stripStageDirections(displayText);
  const spokenText = plan.needsTranslation
    ? await port.translate(clean, plan.ttsLang)
    : clean;
...
export function makeTokenStreamReadout(
  ...
  return {
    onToken: (token: string): void => {
      ...
      const tail = splitter.flush();
      if (tail !== null) {
        const clean = stripStageDirections(tail);
        if (clean.length > 0) session.push(clean);
      }
      session.finish();
    },
...
/** 一次 push 的分流结果:哨兵前(显示段增量)与哨兵后(口语段增量)。 */
export interface DualSplitDelta {
  /** 本次新增的「显示段」字符(推气泡)。 */
  readonly display: string;
  /** 本次新增的「口语段」字符(喂 TTS,哨兵后)。 */
  readonly spoken: string;
}

/**
 * 双语原生输出流式分流器(纯,可测,§4.1 / design D4 + 音频优先排序):把一条 token 流按
 * {@link DUAL_OUTPUT_SENTINEL} 切成「口语段(哨兵**前**,先出→喂 TTS)」与「显示段(哨兵**后**→气泡)」。
 * **音频优先**:口语在前,首音最快。**处理哨兵被任意 token 边界切分**——用"可能是哨兵前缀"的尾缓冲挂起
 * 未确认字符,确保哨兵被拆成两半也能识别、不漏不误喂。哨兵自身既不进口语也不进显示。
 */
export class DualOutputSplitter {
  #phase: 'spoken' | 'display' = 'spoken'; // 音频优先:口语段先出。
  #pending = ''; // 口语段尾部"可能是哨兵前缀"的挂起字符(未确认)。
  #sawSentinel = false;
  readonly #sentinel: string;

  constructor(sentinel: string = DUAL_OUTPUT_SENTINEL) {
    this.#sentinel = sentinel;
  }

  /** 是否已遇到哨兵(切到显示段);供降级判定(未遇 → 模型没按双语格式,前段未必是真口语)。 */
  get sawSentinel(): boolean {
    return this.#sawSentinel;
  }

  /** 喂一段 token,返回本次的显示/口语增量。 */
  push(token: string): DualSplitDelta {
    if (this.#phase === 'display') return { display: token, spoken: '' };
    const combined = this.#pending + token;
    const idx = combined.indexOf(this.#sentinel);
    if (idx >= 0) {
      this.#phase = 'display';
      this.#sawSentinel = true;
      this.#pending = '';
      return { spoken: combined.slice(0, idx), display: combined.slice(idx + this.#sentinel.length) };
    }
    // 未命中:除"可能是哨兵前缀的尾缀"外都可安全作口语输出(喂 TTS)。
    const keep = longestSuffixPrefixLen(combined, this.#sentinel);
    this.#pending = combined.slice(combined.length - keep);
    return { spoken: combined.slice(0, combined.length - keep), display: '' };
  }

  /** 流结束:吐挂起的尾部。仍在口语段(没出现哨兵)→ 挂起字符归口语;否则无残余。 */
  flush(): DualSplitDelta {
    if (this.#phase === 'spoken') {
      const spoken = this.#pending;
      this.#pending = '';
...
/** combined 的最长后缀长度,使该后缀是 sentinel 的前缀(用于跨 token 哨兵边界挂起)。 */
function longestSuffixPrefixLen(combined: string, sentinel: string): number {
  const max = Math.min(combined.length, sentinel.length - 1);
  for (let k = max; k > 0; k--) {
    if (sentinel.startsWith(combined.slice(combined.length - k))) return k;
  }
  return 0;
}

/** makeDualOutputReadout 句柄:边生成边分流——口语段(先出)逐句流式喂 TTS,显示段(后出)经 pushToken 推气泡。 */
export interface DualOutputReadout {
  /** 接进 `convo.send` 的 onToken:返回本次应推给气泡的显示增量(口语段已内部喂 TTS,不返回)。 */
  readonly pushToken: (token: string) => string;
  /** 回合结束:flush 分流尾 + 句切残余 + finish 会话;返回显示文本(合规=哨兵后显示段;非合规=整段当显示)。 */
  readonly done: () => string;
  /** 是否真按双语格式产出口语段(需见哨兵;false → 模型没按格式,前段未必真口语 → 调用方降级翻译通道)。 */
  readonly sawSpoken: () => boolean;
  /** 音频消费完成(正常/出错/abort)的 Promise;上层据此降级。 */
  readonly consumed: Promise<void>;
}

/**
 * 双语原生输出「边生成边分流喂 TTS」(纯,§4.1 / design D7 + 音频优先排序):一条 token 流 →
 * **口语段(哨兵前,先出)逐句流式喂同一 TTS 会话** → 首句即出声、**取代第二次翻译调用**;显示段(哨兵后)推气泡。
 *
 * **乐观流式**(音频优先,§用户定夺):口语段一边生成一边喂 TTS(不等哨兵确认),首音最快。
 * **非合规兜底**:整段无哨兵(模型没按双语格式,前段其实是普通回复)→ `done()` abort 在途音频、把整段当显示文本返回,
 * `sawSpoken()` 返 false → 调用方回落 translateForSpeech 重新正确合成(§3.2 优雅降级,不删翻译通道)。
 * 口语段再经 stripStageDirections 兜一道(双语指令已要求纯口语,这里防漏)。
 */
export function makeDualOutputReadout(
  port: TokenStreamReadoutPort,
  ttsLang: string,
  signal: AbortSignal,
): DualOutputReadout {
  const dual = new DualOutputSplitter();
  const sentence = port.newSplitter();
  const session = port.openSession(ttsLang);
  let displayAcc = '';
  let spokenRaw = ''; // 口语段原始累积:非合规(无哨兵)时整段回落为显示文本。
  let fedAny = false;
  const onAbort = (): void => session.abort();
  if (signal.aborted) session.abort();
  else signal.addEventListener('abort', onAbort, { once: true });

  const feedSpoken = (text: string): void => {
    if (text.length === 0 || signal.aborted) return;
    spokenRaw += text;
    for (const s of sentence.push(text)) {
  ...
  return {
    pushToken: (token: string): string => {
      if (signal.aborted) return '';
      const { display, spoken } = dual.push(token);
      feedSpoken(spoken);
      if (display.length > 0) displayAcc += display;
      return display;
    },
    done: (): string => {
      const tail = dual.flush();
      feedSpoken(tail.spoken);
      if (tail.display.length > 0) displayAcc += tail.display;
      if (dual.sawSentinel) {
        // 合规:口语段已喂完,flush 句切残余 + finish;显示文本 = 哨兵后显示段。
        if (!signal.aborted) {
          const st = sentence.flush();
          if (st !== null) {
            const clean = stripStageDirections(st);
            if (clean.length > 0) {
              session.push(clean);
              fedAny = true;
            }
        ...
        if (fedAny && !signal.aborted) session.finish();
        else session.abort();
        return displayAcc;
      }
      // 非合规(无哨兵):喂进 TTS 的其实是普通回复(可能非合成语种)→ 弃乐观音频,整段当显示文本,交调用方降级翻译。
      session.abort();
      return spokenRaw.trim();
    },
    sawSpoken: (): boolean => fedAny && dual.sawSentinel,
    consumed,
  };
}
